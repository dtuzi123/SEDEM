...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.farr1, self.flabelArr1, self.farr2, self.flabelArr2, self.farr3, self.flabelArr3, self.farr4, self.flabelArr4, self.farr5, self.flabelArr5 = Split_dataset_by5(
            x_train,
            y_train)

        self.farr1_test, self.flabelArr1_test, self.farr2_test, self.flabelArr2_test, self.farr3_test, self.flabelArr3_test, self.farr4_test, self.flabelArr4_test, self.farr5_test, self.flabelArr5_test = Split_dataset_by5(
            x_test,
            y_test)
        ...
        totalSet = np.concatenate((self.arr1, self.arr2, self.arr3, self.arr4, self.arr5, self.farr1, self.farr2,
                                   self.farr3, self.farr4, self.farr5),
                                  axis=0)
        ...
        totalTest = np.concatenate((self.arr1_test, self.arr2_test, self.arr3_test, self.arr4_test, self.arr5_test,
                                    self.farr1_test, self.farr2_test, self.farr3_test, self.farr4_test,
                                    self.farr5_test),
                                   axis=0)
        self.totalTestingSet = totalTest

        self.data_textX = np.concatenate((
            self.arr1_test, self.arr2_test, self.arr3_test, self.arr4_test, self.arr5_test,
            self.farr1_test, self.farr2_test, self.farr3_test, self.farr4_test,
            self.farr5_test), axis=0)
        self.data_textY = np.concatenate((self.labelArr1_test, self.labelArr2_test, self.labelArr3_test,
                                          self.labelArr4_test, self.labelArr5_test, self.flabelArr1_test,
                                          self.flabelArr2_test, self.flabelArr3_test, self.flabelArr4_test,
                                          self.flabelArr5_test), axis=0)

        totaltestLabel = np.concatenate((self.labelArr1_test, self.labelArr2_test, self.labelArr3_test,
                                         self.labelArr4_test, self.labelArr5_test, self.flabelArr1_test,
                                         self.flabelArr2_test, self.flabelArr3_test, self.flabelArr4_test,
                                         self.flabelArr5_test), axis=0)

        totalLabel = np.concatenate((self.labelArr1, self.labelArr2, self.labelArr3, self.labelArr4, self.labelArr5,
                                     self.flabelArr1, self.flabelArr2, self.flabelArr3, self.flabelArr4,
                                     self.flabelArr5),
                                    axis=0)
    ...
    def Image_classifier(self,inputs, scopename, is_training=True, reuse=False):
        with tf.variable_scope(scopename, reuse=reuse):
            batch_norm_params = {'is_training': is_training, 'decay': 0.9, 'updates_collections': None}
            with slim.arg_scope([slim.conv2d, slim.fully_connected],
                                normalizer_fn=slim.batch_norm,
    ...
    def Accumulated_Image_classifier(self,inputs, scopename, is_training=True, reuse=False):
        with tf.variable_scope(scopename, reuse=reuse):
            batch_norm_params = {'is_training': is_training, 'decay': 0.9, 'updates_collections': None}
            with slim.arg_scope([slim.conv2d, slim.fully_connected],
                                normalizer_fn=slim.batch_norm,
                ...
                f = net
                weights = tf.Variable(
                    tf.random_normal(shape=[np.shape(self.accumulatedFeatures)[0] + 1], mean=0, stddev=1),
    ...
    def small_encoder(self,name, x, z_dim=20, reuse=False):
        with tf.variable_scope(name, reuse=reuse):
            l1 = tf.layers.dense(x, 1000, activation=tf.nn.tanh)
            l2 = tf.layers.dense(l1, 500, activation=tf.nn.tanh)
            # l2 = tf.layers.dense(l1, 200, activation=tf.nn.relu)
            z_dim = 50
    ...
    def small_decoder(self,name,z, z_dim=50, reuse=False):
        with tf.variable_scope(name, reuse=reuse):
            ...
            l3 = tf.layers.dense(l2, 500, activation=tf.nn.relu)
            # l2 = tf.layers.dense(l1, 200, activation=tf.nn.relu)
            x_hat = tf.layers.dense(
                l3, 4096, activation=None)
        return x_hat
    ...
    def Create_VAEs(self,index,selectedComponentIndex):
        ...
        myClassifierName = "myClassifier" + str(index)
        ...
        if self.componentCount == 0:
            ...
            logits, feature = self.Image_classifier(self.inputs, myClassifierName, reuse=False)
            ...
            var_classifier = [var for var in T_vars if var.name.startswith(myClassifierName)]

            small1 = [var for var in T_vars if var.name.startswith(smallEncoderName)]
            small2 = [var for var in T_vars if var.name.startswith(smallDecoderName)]
            var_small = small1 + small2

            with tf.variable_scope("foo", reuse=tf.AUTO_REUSE):
                Teacher_optim1 = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(TeacherClassLoss, var_list=var_classifier)
                self.ClassifierOptimArr.append(Teacher_optim1)

                self.smallVAE = tf.train.AdamOptimizer(learning_rate=self.learning_rate) \
                    .minimize(myLoss, var_list=var_small)
            ...
            logits, feature = self.Accumulated_Image_classifier(self.inputs, myClassifierName, reuse=False)
            label_softmax = tf.nn.softmax(logits)
            predictions = tf.argmax(label_softmax, 1)
            self.TeacherPredictions_probs.append(label_softmax)
            self.TeacherPredictions.append(predictions)
    ...
    def build_model(self):
        ...
        self.feature_input1 = tf.placeholder(tf.float32, [self.batch_size, 1024])
        self.feature_input2 = tf.placeholder(tf.float32, [self.batch_size, 1024])
    ...
    def SelectComponentByBatch(self,batch):
        losses = []
        for i in range(np.shape(self.featureRecoArr)[0]):
            loss1 = self.sess.run(self.featureRecoArr[i], feed_dict={self.inputs: batch})
            losses.append(loss1)
    ...
    def Predictions_By_Index2(self,testX,index):
        ...
        #Selection
        lossArr = []
        for t2 in range(np.shape(self.featureRecoArr)[0]):
            sumLoss = 0
            for t1 in range(myN):
                newbatch = testX[t1*self.batch_size:(t1+1)*self.batch_size]
                loss1 = self.sess.run(self.featureRecoArr[t2],feed_dict={self.inputs:newbatch})
                sumLoss = sumLoss + loss1
            sumLoss = sumLoss / myN
        ...
        for i in range(myN):
            ...
            predictions = self.sess.run(self.TeacherPredictions[minIndex],feed_dict={self.inputs:my1})
            #predictions = np.argmax(predictions,axis=1)
            for k in range(self.batch_size):
                totalPredictions.append(predictions[k])
        totalPredictions = np.array(totalPredictions)
    ...
    def Calculate_Feature_CosDistance(self,codeArr1,codeArr2):
        ...
        sum1 = 0
        for i in range(np.shape(codeArr1)[0]):
            m = self.cos_sim(codeArr1[i],codeArr2[i])
            sum1 = sum1 + m

        sum1 = sum1 / np.shape(codeArr1)[0]
        return sum1
    ...
    def Calculate_SelectionScore_BySingle(self,single,singleLabel):
        batch1 = np.tile(single,(self.batch_size,1,1,1))
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
                ...
                if self.componentCount == 1:
                    ...
                    for epoch in range(epochs):
                        ...
                        for idx in range(myCount):
                            ...
                            _,_ = self.sess.run([self.ClassifierOptimArr[self.componentCount-1],self.smallVAEArr[self.componentCount-1]],
            ...
            facc1 = self.Calculate_Accuracy_ByIndex(self.farr1_test,self.flabelArr1_test,0)
            facc2 = self.Calculate_Accuracy_ByIndex(self.farr2_test,self.flabelArr2_test,0)
            facc3 = self.Calculate_Accuracy_ByIndex(self.farr3_test,self.flabelArr3_test,0)
            facc4 = self.Calculate_Accuracy_ByIndex(self.farr4_test,self.flabelArr4_test,0)
            facc5 = self.Calculate_Accuracy_ByIndex(self.farr5_test,self.flabelArr5_test,0)

            acc = acc1 + acc2 + acc3 + acc4 + acc5 + facc1 + facc2 + facc3 + facc4 + facc5
            acc = acc / 10.0
            print(acc)
