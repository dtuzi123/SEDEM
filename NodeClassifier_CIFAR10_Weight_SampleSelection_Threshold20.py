    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for t1 in range(int(np.shape(self.totalSet)[0] / self.data_stream_batch)):
                ...
                if t1 > addCount:
                    if np.shape(self.TeacherFeatures)[0] == 1:
                ...
                if np.shape(self.FixedMemory)[0] > self.maxMmeorySize:
                    ...
                    if np.shape(self.TeacherFeatures)[0] > 1 and runCount > myCountIndex:
                        # Check Expansion
                        print("Check")
                        distance = self.Calculate_Preditive_Distance()
            ...
            myThirdName = "results/SplitCIFAR10_class.txt"
            ...
            myThirdName = "results/SplitCIFAR10_components.txt"
            f = open(myThirdName, "w", encoding="utf-8")
            for i in range(np.shape(arr1)[0]):
                f.writelines(arr1[i])
                f.writelines('\n')
